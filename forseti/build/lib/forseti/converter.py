...
def convert_formula_dnf(statement):
    """
    Converts a given formula into something we can use
    as DNF clauses for reverse resolution

    :param statement:
    :return: a DNF statement
    """
    if isinstance(statement, Symbol):
        return statement
    elif not isinstance(statement, Formula):
    ...
    while not _is_dnf(statement):
        statement = _distribute_and(statement)
...
def _distribute_and(statement):
    """
    Distribute and over any nested and statements (so or becomes
    outer most logical operator)
    ...
    if isinstance(statement, And):
        for i in range(len(args)):
            if isinstance(args[i], Or):
                j = (i + 1) % 2
                statement = Or(And(args[i].args[0], args[j]),
                               And(args[i].args[1], args[j]))
                break

    args = statement.args
    for i in range(len(args)):
        args[i] = _distribute_and(args[i])
...
def _is_dnf(statement, has_and=False):
    """
    Is the given formula a DNF yet?
    ...
    if isinstance(statement, Symbol) or isinstance(statement, Herbrand)\
        ...
        has_and = True
    elif isinstance(statement, Or) and has_and is True:
        return False

    args = statement.args
    for i in range(len(args)):
        if _is_dnf(args[i], has_and) is False:
            return False
    ...
    if isinstance(statement, LogicalOperator):
        if isinstance(statement, If):
            if isinstance(statement.args[0], Existential):
                statement = Universal(statement.args[0].symbol, If(
                    statement.args[0].args[0], statement.args[1]))
            elif isinstance(statement.args[0], Universal):
                statement = Existential(statement.args[0].symbol, If(
                    statement.args[0].args[0], statement.args[1]))
            elif isinstance(statement.args[1], Quantifier):
                q_type = type(statement.args[1])
                statement = q_type(statement.args[1].symbol, If(
                    statement.args[0], statement.args[1].args[0]))
        elif isinstance(statement, And):
            if isinstance(statement.args[0], Quantifier):
                q_type = type(statement.args[0])
                statement = q_type(statement.args[0].symbol, And(
                    statement.args[0].args[0], statement.args[1]))
            elif isinstance(statement.args[1], Quantifier):
                q_type = type(statement.args[1])
                statement = q_type(statement.args[1].symbol, And(
                    statement.args[0], statement.args[1].args[0]))
        elif isinstance(statement, Or):
            if isinstance(statement.args[0], Quantifier):
                q_type = type(statement.args[0])
                statement = q_type(statement.args[0].symbol, Or(
                    statement.args[0].args[0], statement.args[1]))
            elif isinstance(statement.args[1], Quantifier):
                q_type = type(statement.args[1])
                statement = q_type(statement.args[1].symbol, Or(
                    statement.args[0], statement.args[1].args[0]))
...
# print(convert_formula(parser.parse("if(A,and(B,C))")))
# print(convert_formula_dnf(parser.parse("not(if(A,and(B,C)))")))
